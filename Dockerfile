...
# Copy the uacapp service (converter base exe + web UI + converter logic)
COPY --chown=app:app uacapp/ ./uacapp/
...
# Copy the pre-built RepairPro electronics-repair app (built with Vite + React 19)
COPY --chown=app:app electronics-repair/ /app/electronics-repair/
